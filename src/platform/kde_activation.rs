...
pub(crate) fn activate_primary_window(pid: u32) -> bool {
    if !is_kde_session_with(|name| std::env::var_os(name)) {
        return false;
    }
...
mod tests {
    ...
    #[test]
    fn runner_timeout_is_treated_as_activation_failure_and_cleanup_continues() {
        let temp = TestDir::new();
        let mut calls = 0;
        let activated =
            activate_primary_window_with(9, Path::new("/usr/bin/qdbus6"), &temp.path, |_| {
                calls += 1;
